$(function () {
  let digiticket = {
    ...
    renderListFly: function () {
      ...
      <div class="item col">
          <h3 href="${item.moreInfo}" class="link">Chi tiết</h3>
      </div>
      <div class="item col">
          <span class="price">
              ${item.price}
      ...
      <div class="item col">
          <button class="book js-change-html">
              Đặt vé
          </button>
      </div>
          ...
          <div class="setup-fly">
          <div class="row no-gutters bg-white mt8 padl20 padr20 padt20 padb20 align-items-center justify-content-between">
    ...
    coutQuantityOfCustomer: function () {
      $(document).on("click", ".quantity-customer", function (e) {
        e.stopPropagation();
        let $this = $(this);
        let $value = $this.val()
      ...
      $(document).on("change", ".quantity-customer", function (e) {
        let $this = $(this);
        let $value = $this.val();
        let $container = $this.closest(".quantity");
        let $countDiv = $container.find(".quantity-options");
    ...
    changeHtmlWhenBook: function () {
      $(document).on("click", ".js-change-html", function (e) {
        ...
        let $radioInput = $slideDownContainer.find('input[name="ticket-chosen"]')
        ...
        let $thisActive = $(".js-change-html.active").not($this)
        let $allThisNotActive = $('.js-change-html').not('.active')

        console.log($($radioInput).off("click"))
        if($allThisNotActive) {
          ...
          if($($this).hasClass('active')) {
            ...
            // $($radioInput).off("click");
          }
        }
      });
    },
    datePickerCustom: function () {
      // let date2Custom = $('.date-picker-wrapper').last()
      // // $(date2Container).css({"margin-left": "16px",});
      // console.log(date2Custom)
    },
    loadListTicketWhenPickFilter: function() {
      $('.filter-load-list-ticket').click(function (e) { 
        e.preventDefault();
        let $this = $(this)
    ...
    showTicketInfo: function () {
      $(document).on("click", ".fly-info-ticket .link", function (e, index) {
        ...
        let $activeContainer = $this.closest('.fly-info-ticket .even')
        let $activeDropdown = $thisActive.find(".setup-fly");
        let $radio = $container.find('input[name="ticket-chosen"]')
        ...
        if ($thisActive) {
          ...
          $($activeDropdown).slideUp();
          $($activeContainer).toggleClass('active');
        }
      });
    },
    ...
    swapDirection: function () {
      $(document).on("click", ".fly-info .content", function (e) {
        let $this = $(this);
